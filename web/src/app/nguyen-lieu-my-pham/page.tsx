...
export default async function CosmeticIngredients({
  ...
  const cosmeticProducts = await api.getProducts('cosmetic').catch((err) => {
    console.error("Failed to fetch cosmetic products:", err);
    return [];
  });

  cosmeticProducts.sort((a, b) => {
  ...
  const totalItems = cosmeticProducts.length;
  const totalPages = Math.ceil(totalItems / itemsPerPage);
  
  const paginatedProducts = cosmeticProducts.slice(
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {paginatedProducts.map((prod) => (
                  <Link
                    key={prod.id}
                    href={`/nguyen-lieu-my-pham/${prod.slug || prod.id}`}
              ...
              {totalPages > 1 && (
                <div className="flex justify-center items-center gap-2 mt-12">
                  <Link
                    href={`/nguyen-lieu-my-pham?page=${currentPage - 1}`}
                  ...
                  {Array.from({ length: totalPages }).map((_, i) => {
                    ...
                    return (
                      <Link
                        key={pageNumber}
                        href={`/nguyen-lieu-my-pham?page=${pageNumber}`}
                        className={`flex items-center justify-center w-10 h-10 rounded-lg border text-sm font-semibold transition-colors ${
