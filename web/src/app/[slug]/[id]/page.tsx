...
export async function generateMetadata({
  params,
  searchParams,
}: DynamicProductPageProps): Promise<Metadata> {
  ...
  const formattedTitle = name.includes("Sophpower") || name.includes("Sophchem") ? name : `${name} - Sophchem Vietnam`;
  ...
  return {
    ...
    alternates: {
      canonical: product.canonical_url || `${baseUrl}/${slug}/${id}`,
    },
    openGraph: {
      type: "website",
      url: `${baseUrl}/${slug}/${id}`,
...
export default async function DynamicProductDetailPage({
  ...
  let categoryName = getVal(product.category, locale);
  try {
    const categories = await api.getProductCategories();
    const currentCategory = categories.find((c) => c.slug === slug);
    if (currentCategory) {
      categoryName = getVal(currentCategory.name, locale);
    }
  } catch (e) {
  ...
  const allProducts = await api.getProducts(product.type).catch(() => []);
  const relatedList = allProducts
    .filter((item) => String(item.id) !== String(product.id) && item.slug !== id)
    .slice(0, 3);

  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000";
  const productUrl = `${baseUrl}/${slug}/${product.slug || product.id}`;
        ...
        <ScrollReveal direction="up" delay={150} duration={600} className="w-full lg:w-[55%]">
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div
                className="rich-text text-gray-605 leading-relaxed text-justify text-sm sm:text-base"
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-6">
              ...
              >
                ...
                {isFood ? t.products.inquiryFoodButton : t.products.inquiryCosmeticButton}
              </Link>
              <a
